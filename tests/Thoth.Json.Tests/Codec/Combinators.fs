...
let tests (runner: TestRunner<'DecoderJsonValue, 'EncoderJsonValue>) =
    testList (
        ...
        [
            test (
                "Codec.lossyOption works with Some value",
                fun _ ->
            ...
            test (
                "Codec.lossyOption works with None",
                fun _ ->
                    let codec = Codec.lossyOption Codec.string
            ...
            test (
                "Codec.oneOf decodes a fallback representation",
                fun _ ->
                    let codec =
                        Codec.oneOf
                            [
            ...
            test (
                "Codec.oneOf reports an error when no codec matches",
                fun _ ->
                    let codec =
                        Codec.oneOf
                            [
...
Expecting an array but instead got: true
            ...
            test (
                "Codec.oneOf throws for an empty list",
                fun _ ->
                    assertThat
            ...
            test (
                "Codec.seq reports an error for an invalid element",
                fun _ ->
                    let codec = Codec.seq Codec.int
...
Expecting an int but instead got: "two"
            ...
            test (
                "Codec.resizeArray reports an error for an invalid value",
                fun _ ->
                    let codec = Codec.resizeArray Codec.string
...
Expecting a ResizeArray but instead got: "not an array"
            ...
            test (
                "Codec.dict reports an error for an invalid value",
                fun _ ->
                    let codec = Codec.dict Codec.int
...
Expecting an int but instead got: "not an int"
            ...
            test (
                "Codec.map' reports an error for an invalid value",
                fun _ ->
                    let codec = Codec.map' Codec.int Codec.string
...
Expecting a string but instead got: 2
            ...
            test (
                "Codec.value round-trips raw JSON",
                fun _ ->
                    let expected =
                        Json.Object
                            [
            ...
            test (
                "Codec.nil reports an error for a non-null value",
                fun _ ->
                    let codec = Codec.nil 42
...
Expecting null but instead got: 5
                    ...
                    let actual =
                        runner.Decode.fromString (Decode.codec codec) "5"

                    equal actual expected
            )

            test (
                "Codec.keyValuePairs works",
                fun _ ->
            ...
            test (
                "Codec.keyValuePairs reports an error for an invalid value",
                fun _ ->
                    let codec = Codec.keyValuePairs Codec.int
...
Expecting an int but instead got: "not an int"
            ...
            test (
                "Codec.at reports an error for a missing path",
                fun _ ->
                    let codec =
                        Codec.at
                            [
...
Node `user` is unknown.
                    ...
                    let actual =
                        runner.Decode.fromString (Decode.codec codec) "{}"
            ...
            test (
                "Codec.fix works for a recursive type",
                fun _ ->
                    let expected =
                        Tree(
                            1,
            ...
            test (
                "Codec.value |> Codec.map builds a custom codec",
                fun _ ->
